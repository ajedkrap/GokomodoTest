...
const StarshipsDetail = ({route, navigation: nav}) => {
  const [isLoading, setLoading] = useState(true);
  const [data, setData] = useState(null);

  const onGettingData = async () => {
    try {
      const gettingStarships = await getKeyBasedData(route.params.url);
      setData(gettingStarships);
    } catch (e) {
        ...
        <View style={{paddingHorizontal: 16}}>
          ...
          <View style={{flexDirection: 'row', minHeight: 120}}>
            <View
              ...
              <Icon name={'rocket'} size={72} color={color.primary} />
            </View>
